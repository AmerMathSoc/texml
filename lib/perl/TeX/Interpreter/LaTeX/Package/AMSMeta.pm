package TeX::Interpreter::LaTeX::Package::AMSMeta;
...
sub install {
    ...
    # We might need, for example, \tsup, but we can't load textcmds
    # without causing too many conflicts with author macros.  See
    # below.

    # $tex->load_package("textcmds");

    $tex->read_package_data(*TeX::Interpreter::LaTeX::Package::AMSMeta::DATA{IO});
...
sub append_article_notes {
    ...
    my @dedications = $gentag->get_dedications();

    return unless @dedications;
