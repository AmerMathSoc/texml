...
\T{mem} (the \emph{dynamic} memory) is an array of \T{memory_word}:
...
\cline{1-1}
null (= min_halfword) \\ \cline{1-1}
\quad\vdots\\\cline{1-1}
mem_min & start of variable-size nodes \\ \cline{1-1}
\quad\vdots\\\cline{1-1}
mem_bot \\ \cline{1-1}
\quad\vdots\\\cline{1-1}
lo_mem_max\\\cline{1-1}
\quad\vdots\\
\textnormal{\emph{unallocated}}\\
\quad\vdots\\\cline{1-1}
hi_mem_min & start of one-word records
\\ \cline{1-1}
\quad\vdots\\
\textnormal{\emph{character nodes}}\\
\quad\vdots\\\cline{1-1}
mem_top
\\ \cline{1-1}
\quad\vdots\\\cline{1-1}
...
In \texttt{initex}, $\mathtt{mem_min} = \mathtt{mem_bot}$ and
$\mathtt{mem_max} = \mathtt{mem_top}$, and entries in the range
\texttt{mem_bot..mem_top} are dumped to the \texttt{.fmt} file.
\[
\mathtt{null} \le \mathtt{mem_min} \le \mathtt{mem_bot} < \mathtt{lo_mem_max}
<
  \mathtt{hi_mem_min} < \mathtt{mem_top} \le \mathtt{mem_end} \le \mathtt{mem_max}.
\]
